...
const LiveSession: React.FC<LiveSessionProps> = ({ songs, initialIndex = 0, onExit, onSongUpdate }) => {
  ...
  if (!currentSong) return <div className="p-8 text-center text-slate-500">No songs selected for live session.</div>;
  ...
  return (
    <div 
        className="fixed inset-0 bg-white text-slate-900 z-50 flex flex-col h-screen overflow-hidden"
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
    >
      {/* Top Bar - Sticky */}
      <div 
        className={`flex flex-col border-b border-slate-200 bg-white shadow-sm transition-all duration-300 z-20 ${!showControls ? '-translate-y-full absolute w-full opacity-0 pointer-events-none' : 'translate-y-0 relative opacity-100'}`}
      >
          <div className="flex items-center justify-between p-3 md:p-4">
            <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
                <button onClick={onExit} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-600 shrink-0"><X size={24} /></button>
                <div className="min-w-0">
                    <div className="flex items-baseline gap-2">
                        <h1 className="text-lg md:text-xl font-bold leading-tight text-slate-900 truncate">{currentSong.title}</h1>
                        <span className="text-xs text-slate-400 whitespace-nowrap hidden sm:inline-block">({currentIndex + 1} of {songs.length})</span>
                    </div>
            ...
            <div className="flex items-center gap-2 md:gap-4 shrink-0">
                {/* Capo Control */}
                 <div className="hidden md:flex flex-col items-center bg-slate-50 rounded-lg border border-slate-200 px-2 py-1">
                     <span className="text-[10px] text-slate-400 uppercase font-bold">Capo</span>
                     <div className="flex items-center gap-2">
                        <button onClick={() => setCapo(c => Math.max(0, c - 1))} className="text-slate-400 hover:text-slate-600 disabled:opacity-30" disabled={capo === 0}><Minus size={14}/></button>
                        <span className={`font-bold w-4 text-center ${capo > 0 ? 'text-accent' : 'text-slate-300'}`}>{capo}</span>
                        <button onClick={() => setCapo(c => Math.min(11, c + 1))} className="text-slate-400 hover:text-slate-600"><Plus size={14}/></button>
                     </div>
                 </div>
                ...
                <div className="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200">
                    <button 
                        ...
                        className="p-2 hover:bg-white rounded shadow-sm transition-all text-slate-600"
                    ...
                    <div className="px-3 flex flex-col items-center justify-center min-w-[3.5rem]">
                        ...
                        {/* Visual indicator for Capo/Play Key */}
                        {capo > 0 ? (
                            <div className="flex items-center gap-1 mt-0.5 animate-in fade-in zoom-in">
                                <span className="text-[9px] text-slate-400 font-bold uppercase">Play</span>
                                <span className="text-xs font-bold text-accent bg-accent/10 px-1 rounded">{displayKey}</span>
                            </div>
                        ) : (
                             <span className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">Key</span>
                        )}
                    </div>
                ...
                >
                    <span className="text-[9px] uppercase font-bold text-slate-400 mb-0.5">Simplify</span>
                    <div className={`w-9 h-5 rounded-full relative transition-colors duration-200 ${simplify ? 'bg-primary' : 'bg-slate-300'}`}>
                ...
                <div className="hidden lg:flex items-center bg-slate-100 rounded-lg p-1 text-sm border border-slate-200 text-slate-600 font-medium">
                    <button onClick={() => setFontSize('base')} className={`px-2 py-1 rounded ${fontSize === 'base' ? 'bg-white shadow-sm' : ''}`}>A</button>
                    <button onClick={() => setFontSize('lg')} className={`px-2 py-1 rounded ${fontSize === 'lg' ? 'bg-white shadow-sm' : ''}`}>A+</button>
                    <button onClick={() => setFontSize('xl')} className={`px-2 py-1 rounded ${fontSize === 'xl' ? 'bg-white shadow-sm' : ''}`}>A++</button>
                </div>
            </div>
          </div>
      </div>
      ...
      >
        <div className="sm:hidden mb-6 pb-4 border-b border-slate-200">
             <div className="flex justify-between items-start">
                 <div>
                    <h1 className="text-3xl font-black leading-tight text-slate-900 mb-1">{currentSong.title}</h1>
                    <p className="text-slate-500 font-bold">{currentSong.artist}</p>
                 </div>
                 <div className="text-right">
                     <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-full">{currentIndex + 1}/{songs.length}</span>
                 </div>
             </div>
             {capo > 0 && (
                <div className="mt-3 flex items-center gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
                    <span className="text-sm font-bold text-slate-500">Capo {capo}</span>
      ...
      <div className={`absolute bottom-6 left-0 right-0 px-4 md:px-8 pointer-events-none transition-opacity duration-300 z-50 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
         <div className="flex items-end justify-between max-w-5xl mx-auto">
            ...
            <div className="pointer-events-auto">
                {currentIndex > 0 && (
                     <button 
                        ...
                        className="bg-white border border-slate-200 text-slate-700 p-4 rounded-full shadow-lg hover:bg-slate-50 transform transition-all active:scale-95 flex items-center gap-2 self-start"
                     >
                        <ChevronLeft size={24} />
            ...
            <div className="pointer-events-auto flex flex-col items-end gap-3">
                 {/* Auto Scroll Widget - Removed animate-in to prevent mobile visibility issues */}
                 <div className="bg-white/90 backdrop-blur border border-slate-200 p-2.5 rounded-xl shadow-lg flex flex-col gap-2 items-end mb-2">
                     <div className="text-[10px] uppercase font-bold text-slate-400 px-1 flex items-center gap-1">
